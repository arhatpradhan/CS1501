...
public class PHPArray<V> implements Iterable<V> {
  ...
  public void put(String key, V val) {
    ...
    //if list is empty then head needs to be set to the 1st node and tail needs to be set to the first node
    //else tail needs to be set to the last node and 
    if(head == null){
      head = entries[i];
      tail = head;
      tail.next = null; 
  ...
  private void resize(int capacity) {
    ...
    Node a = head;
    while(a != null){
      //while the node is not null get entry from that node and insert it into temp 
      temp.put(a.key, (V) a.value);
      a = a.next;
    }
