    ...
    public static void compress() {
      ...
      //StringBuilder instead of String
        ...
        while (!BinaryStdIn.isEmpty()) {
            ...
            //if current is not found in the symbol table add it to the table
            if(!st.contains(current)){
              BinaryStdOut.write(codeword, W);
              if (code < L)    // Add to symbol table if not full
                  st.put(current, code++);
              //TODO: reset current <><>
              StringBuilder reset = new StringBuilder(Character.toString(c));
              current = reset;
            }
        }
        ...
        //in current
        //if it has any unprocessed characters get the corresponding codeword 
        //and write that value to the file
        if (current.length() != 0) BinaryStdOut.write(st.get(current), W);
