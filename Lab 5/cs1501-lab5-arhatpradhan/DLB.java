    ...
    ****************************************************/
        ...
        if(x != null){
          if(x.letter == key.charAt(pos)){
            if(pos == key.length()-1){
              ...
              result = get(x.child, key, ++pos);/*TODO: Recurse on the child node*/;
            }
          } else {
    ...
    private Node put(Node x, String key, Value val, int pos) {
        ...
        if (x == null){
            ...
            result.level = pos;
            if(pos < key.length()-1){
              result.child = put(x , key, val, ++pos); /*TODO: Recurse on the child node*/;
            } else {
              result.val = val;
            }
            ...
            if(pos < key.length()-1){
              ...
              result.child = put(x.child , key, val, ++pos);
            } else {
