...
public class PHPArray<V> implements Iterable<V> {
  ...
  public void put(String key, V val) {
    ...
    // if head is null then just add into the front
    if (head == null) {
      head = entries[i];
      tail = entries[i];
    } else {
      // if head is not null
      // current tail.next = new entry
      tail.next = entries[i];
      // new entry. prev = tail
      entries[i].prev = tail;
      // set tail to new entry
      tail = entries[i];
    }
    N++;
  }
  ...
  public void unset(String key) {
    ...
    if (toDelete == head && toDelete == tail) { // if only node in list
      head = null;
      tail = null;
